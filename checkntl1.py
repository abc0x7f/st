...
def draw_gradient_histogram(
    ax, values, title, xlabel, cmap_name, kde_color, clip_negative=False
):
    if clip_negative:
    ...
    ax_density = ax.twinx()
    kw = dict(color=kde_color, linewidth=2.2, label="核密度曲线", ax=ax_density)
    if clip_negative:
        kw["clip"] = (0, values.max())
...
def save_distribution_plot(df):
    ...
    draw_gradient_histogram(axes[1], df["lntl"], "lntl 分布", "lntl",
                            "Oranges", "#C65D00", clip_negative=False)
...
def _draw_scale_bar(ax, x, y, bar_km, lat_ref):
    ...
    ax.text(x, y - 0.3, "0", ha="center", va="top", fontsize=7)
    ax.text(x + half, y - 0.3, f"{bar_km // 2:.0f}", ha="center", va="top", fontsize=7)
    ax.text(x + bar_deg, y - 0.3, f"{bar_km:.0f} km", ha="center", va="top", fontsize=7)
...
def save_lntl_map(df):
    ...
    for sp in ax.spines.values():
        ...
        sp.set_color("black")
    ax.set_title(f"{MAP_YEAR} 年各省 lntl 分级图", fontsize=16,
                 fontweight="bold", pad=12)

    # ══════════════ South China Sea inset (右下角) ══════════════
    ax_scs = fig.add_axes([0.745, 0.11, 0.14, 0.22])
    scs_p, scs_fc = [], []
    # Hainan in inset
    if hainan_feature:
    ...
    sx = 1.0
    half_b = bar_leg / 2
    ax_leg.add_patch(Rectangle((sx, sy), half_b, 0.45,
                               fc="black", ec="black", lw=0.8))
    ax_leg.add_patch(Rectangle((sx + half_b, sy), half_b, 0.45,
                               fc="white", ec="black", lw=0.8))
    ax_leg.text(sx, sy - 0.25, "0", ha="center", fontsize=6.5)
    ax_leg.text(sx + half_b, sy - 0.25, f"{bar_km // 2}", ha="center", fontsize=6.5)
    ax_leg.text(sx + bar_leg, sy - 0.25, f"{bar_km} km", ha="center", fontsize=6.5)

    # ── 3) Segmented colour legend ──
    box_w, box_h = 2.0, 1.1
    lx = 1.0
    ly_start = 11.5      # start y for top segment
