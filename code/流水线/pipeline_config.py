...
STEP_DEFINITIONS: tuple[StepDefinition, ...] = (
    ...
    StepDefinition(
        ...
        expected_outputs=("outputs/空间分析/30_空间权重矩阵检验/*",),
        primary_csv=root_path("outputs", "空间分析", "30_空间权重矩阵检验", "LR检验结果.csv"),
        markdown_globs=("outputs/空间分析/30_空间权重矩阵检验/*.md",),
    ...
    StepDefinition(
        ...
        expected_outputs=("outputs/空间分析/40_空间SDM主模型/*",),
        primary_csv=root_path("outputs", "空间分析", "40_空间SDM主模型", "主模型系数表.csv"),
        markdown_globs=("outputs/空间分析/40_空间SDM主模型/*.md",),
